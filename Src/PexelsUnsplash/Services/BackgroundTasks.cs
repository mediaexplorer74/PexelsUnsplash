...
namespace Stream.Wall.Services {
    public class BackgroundTasks {
        ...
        public static ApplicationDataCompositeValue GetTileTaskActivity() {
            ...
            var settingsValues = ApplicationData.Current.LocalSettings.Values;
            var b = settingsValues.ContainsKey(key) ? (ApplicationDataCompositeValue)settingsValues[key] : null;
            return b;
        }
        #endregion tile task
        ...
        public static ApplicationDataCompositeValue GetWallTaskActivity() {
            ...
            var settingsValues = ApplicationData.Current.LocalSettings.Values;
            var test = settingsValues.ContainsKey(key);
            if (test)
            {
                var bs =  (ApplicationDataCompositeValue) settingsValues[key];
                if (bs == null)
                    return null;
        ...
        public static ApplicationDataCompositeValue GetLockscreenTaskActivity() {
            ...
            var test = settingsValues.ContainsKey(key);
            if (test)
                return (ApplicationDataCompositeValue) settingsValues[key];
        ...
        private static async void RegisterBackgroundTask(string taskName, string entryPoint, uint interval) {
            ...
            var builder = new BackgroundTaskBuilder() {
                Name = taskName,
                TaskEntryPoint = typeof(WallUpdater).FullName
            };
